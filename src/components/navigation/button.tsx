...
function Button({
    ...
    return (
        <button
            onClick={onClick}
            className="text-xl cursor-pointer text-fg-2 hover:text-fg transition-colors duration-75"
        >
            {children}
        </button>
    );
...
function Link({ path, children }: { path: string; children: ReactNode }) {
    ...
    return (
        <RouterLink
            to={path}
            className={`text-xl cursor-pointer ${matches ? "text-fg" : "text-fg-2 hover:text-fg transition-colors duration-75"}`}
        >
            {children}
        </RouterLink>
    );
